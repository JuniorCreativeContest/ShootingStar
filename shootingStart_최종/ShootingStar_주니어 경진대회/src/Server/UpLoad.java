...
public class UpLoad{
    ...
    public boolean uploadFile(String path,String id, String type){
        OutputStream out;
        FileInputStream fin;
        ...
        try{
            ...
            DataOutputStream dout = new DataOutputStream(out); //OutputStream을 이용해 데이터 단위로 보내는 스트림을 개통합니다.
              ...
              //파일 이름을 입력받기위해 스캐너를 생성합니다.
            
            while(true){
            String filename = path;    //스캐너를 통해 파일의 이름을 입력받고,
            fin = new FileInputStream(new File(filename)); //FileInputStream - 파일에서 입력받는 스트림을 개통합니다.
            
        byte[] buffer = new byte[8096];        //바이트단위로 임시저장하는 버퍼를 생성합니다.
        ...
        fin.close();
        int getPosition = filename.lastIndexOf("\\");
        String newFileName = "\\"+id+"\\"+type+filename.substring(getPosition);
        fin = new FileInputStream(filename);   //FileInputStream이 만료되었으니 새롭게 개통합니다.
        dout.writeInt(data);                   //데이터 전송횟수를 서버에 전송하고,
        dout.writeUTF(newFileName);               //파일의 이름을 서버에 전송합니다.
